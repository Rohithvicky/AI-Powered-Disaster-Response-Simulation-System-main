...
class DisasterSimulator:
    ...
    def _generate_hazards(self, disaster_type: str) -> Dict[Tuple[int, int], float]:
        ...
        if disaster_type == "earthquake":
            ...
            center = (random.randint(self.grid_size//4, 3*self.grid_size//4),
                     random.randint(self.grid_size//4, 3*self.grid_size//4))
            for i in range(self.grid_size):
                for j in range(self.grid_size):
                    ...
                    spiral_factor = abs(math.sin(angle * 3 + dist * 0.5))
                    if dist < self.grid_size * 0.35 and random.random() < 0.6:
                        intensity = max(0.1, (1.0 - dist / (self.grid_size * 0.35)) * spiral_factor)
                        hazards[(i, j)] = intensity
        elif disaster_type == "hurricane":
            # Hurricane: large circular area with varying intensity
            center = (random.randint(self.grid_size//4, 3*self.grid_size//4),
                     random.randint(self.grid_size//4, 3*self.grid_size//4))
            for i in range(self.grid_size):
                for j in range(self.grid_size):
                    dist = math.sqrt((i - center[0])**2 + (j - center[1])**2)
                    if dist < self.grid_size * 0.5 and random.random() < 0.8:
                        # Hurricane has multiple intensity bands
                        if dist < self.grid_size * 0.1:
                            intensity = 1.0  # Eye wall
                        elif dist < self.grid_size * 0.3:
                            intensity = max(0.3, 0.9 - (dist - self.grid_size * 0.1) / (self.grid_size * 0.2))
                        else:
                            intensity = max(0.1, 0.3 - (dist - self.grid_size * 0.3) / (self.grid_size * 0.2))
                        hazards[(i, j)] = intensity
        else:
            # Default: random scattered hazards for unknown disaster types
            for i in range(self.grid_size):
                for j in range(self.grid_size):
