...
class DisasterSimulator:
    ...
    def reset(self):
        """Generate new multiple disaster scenario"""
        disaster_types = ["earthquake", "fire", "flood", "hurricane", "tornado"]
        
        # Generate multiple disasters (2-4 disasters)
        num_disasters = random.randint(2, 4)
        active_disasters = random.sample(disaster_types, min(num_disasters, len(disaster_types)))
        ...
        # Generate hazards and resources for each disaster
        for disaster_type in active_disasters:
            disaster_hazards = self._generate_hazards(disaster_type)
            disaster_resources = self._generate_resources(disaster_type)
            
            # Merge hazards (take maximum intensity)
            for pos, intensity in disaster_hazards.items():
                hazards[pos] = max(hazards.get(pos, 0), intensity)
            
            # Add resources
            resources.extend(disaster_resources)
        
        # Remove duplicate resources at same position
        unique_resources = {}
        for r in resources:
            pos = (r[0], r[1])
            if pos not in unique_resources:
                unique_resources[pos] = r
        ...
        self.state = SimulationState(
            ...
            rescue_team=RescueTeam((0, 0), 10), disaster_type="multiple"
        )
        ...
        self.stats = {
            ...
            'disaster_type': "multiple", 'active_disasters': active_disasters
        }
        self.telemetry = {
    ...
    def _check_rescues(self):
        """Check if rescue team can rescue victims"""
        team_pos = self.state.rescue_team.position
        rescued = []
    ...
    def serialize_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_disasters": self.stats.get('active_disasters', []),
            "metrics": self.stats,
