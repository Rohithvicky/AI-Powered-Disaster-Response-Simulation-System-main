...
class DisasterSimulator:
    ...
    def _update_hazards(self):
                ...
                for dr in [-1, 0, 1]:
                    for dc in [-1, 0, 1]:
                        ...
                        if 0 <= nr < self.grid_size and 0 <= nc < self.grid_size:
                            # Calculate spread probability based on intensity and terrain
                            terrain = self.state.grid[nr][nc]
                            spread_prob = 0.8 if intensity > 0.7 else 0.5
                            
                            # Some terrains are more susceptible to certain disasters
                            ...
                            if random.random() < spread_prob:
                                new_intensity = intensity * random.uniform(0.6, 0.9)
                                new_hazards[(nr, nc)] = max(new_hazards.get((nr, nc), 0), new_intensity)
        
        # Intensify existing hazards over time
        for pos in list(new_hazards.keys()):
            if new_hazards[pos] > 0.2:
                # Hazards can intensify or weaken randomly
        ...
        # Add new random hazards occasionally (disaster escalation)
        if random.random() < 0.1:  # 10% chance each step
            self._add_random_hazard()

    def _ai_move(self):
        """AI moves rescue team toward safest victim with disaster avoidance"""
        if not self.state.victims:
            return
        
        team_pos = self.state.rescue_team.position
        
        # Find the safest victim (lowest risk path)
        best_victim = None
        best_risk = float('inf')
        
        for victim in self.state.victims:
            # Calculate path risk to this victim
            path_risk = self._calculate_path_risk(team_pos, victim)
            distance = math.sqrt((team_pos[0] - victim[0])**2 + (team_pos[1] - victim[1])**2)
            
            # Combine risk and distance (prefer closer, safer victims)
            total_cost = path_risk + (distance * 0.1)
        ...
        # Calculate distance to best victim
        distance = math.sqrt((team_pos[0] - best_victim[0])**2 + (team_pos[1] - best_victim[1])**2)
        
        # If already at victim location, don't move
        if distance < 1.0:
            return
        
        # Smart pathfinding: avoid high-risk areas
        new_pos = self._find_safe_path(team_pos, best_victim)
        
        if new_pos == team_pos:
            # Can't move safely, try to find alternative path
            new_pos = self._find_alternative_path(team_pos, best_victim)
        ...
        if 0 <= new_pos[0] < self.grid_size and 0 <= new_pos[1] < self.grid_size:
            ...
            print(f"Rescue team moved from {team_pos} to {new_pos} (target: {best_victim}, risk: {best_risk:.2f})")
    ...
    def _check_rescues(self):
        ...
        for victim in self.state.victims:
            if victim == team_pos and self.state.rescue_team.resources > 0:
                ...
                print(f"🎉 VICTIM RESCUED at {victim} using resources: {used_resources}")
