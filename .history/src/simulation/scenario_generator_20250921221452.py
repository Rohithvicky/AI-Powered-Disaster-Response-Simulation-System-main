...
class ScenarioGenerator:
    ...
    def generate_disaster_scenario(self, size: int) -> Dict[str, Any]:
        """Generate a complete disaster scenario with single disaster type"""
        # Select a single disaster type
        ...
        resources = self.generate_resources(size, disaster_type)
    ...
    def generate_resources(self, size: int, disaster_type: str = None) -> List[Tuple[int, int, str]]:
        """Generate resource positions with specific types based on disaster"""
        if disaster_type is None:
            disaster_type = self.current_disaster_type or "earthquake"
            
        resources = []
        
        # Define resource types based on disaster type
        resource_types = self._get_resource_types_for_disaster(disaster_type)
    ...
    def _get_resource_types_for_disaster(self, disaster_type: str) -> Dict[str, int]:
        """Get appropriate resource types and counts for specific disaster"""
        if disaster_type == "earthquake":
            return {
                "ambulance": 3,
                "rescue_team": 2,
