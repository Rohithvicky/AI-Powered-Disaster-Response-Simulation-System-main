...
class DisasterSimulator:
    ...
    def _update_hazards(self):
        ...
        # Reduce chance as more of the grid becomes hazardous
        hazard_coverage = len(self.state.hazards) / (self.grid_size * self.grid_size)
        escalation_chance = max(0.02, 0.1 - hazard_coverage * 0.5)  # Reduce as coverage increases
        
        if random.random() < escalation_chance:
            self._add_random_hazard()

    def _update_victim_survival(self):
        """Update victim survival probabilities over time"""
        for victim in self.state.victims:
            # Decrease survival probability based on time and hazard intensity
            hazard_intensity = self.state.hazards.get(victim.position, 0)
            time_factor = self.state.time_step - victim.time_discovered
            
            # Survival decreases with time and hazard intensity
            survival_decrease = (time_factor * 0.02) + (hazard_intensity * 0.1)
            victim.survival_probability = max(0.0, victim.survival_probability - survival_decrease)
            
            # Critical victims (injury level 5) lose survival faster
            if victim.injury_level >= 4:
                victim.survival_probability = max(0.0, victim.survival_probability - 0.05)
    ...
    def _find_best_victim_with_path(self, start_pos):
        ...
        for victim in self.state.victims:
            path = self._astar_pathfinding(start_pos, victim)
            if path:
                # Calculate total cost (path length + risk)
                path_risk = sum(self.state.hazards.get(pos, 0) for pos in path[1:])
                total_cost = len(path) + path_risk
    ...
    def _astar_pathfinding(self, start, goal):
        ...
        def get_cost(pos):
            # Cost based on hazard intensity
            hazard_cost = self.state.hazards.get(pos, 0)
            return 1 + hazard_cost * 2  # Base cost + hazard penalty
    ...
    def _emergency_escape(self, current_pos):
        """Emergency escape when no safe path to victims exists"""
