...
class DisasterSimulator:
    ...
    def _ai_move(self):
        ...
        distance = math.sqrt((team_pos[0] - nearest_victim[0])**2 + (team_pos[1] - nearest_victim[1])**2)
        ...
        # Move if the new position is valid (allow movement even through hazards)
        if 0 <= new_pos[0] < self.grid_size and 0 <= new_pos[1] < self.grid_size:
            self.state.rescue_team.position = new_pos
            print(f"Rescue team moved from {team_pos} to {new_pos} (target: {nearest_victim})")
