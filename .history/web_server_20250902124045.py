...
def reset() -> Dict[str, Any]:
    sim.reset()
    return {"ok": True, "state": sim.serialize_state(), "stats": sim.stats, "telemetry": sim.telemetry}
...
def recommend_path() -> Dict[str, Any]:
    info = sim.recommend_path_to_nearest_victim()
    return info
...
def legend() -> Dict[str, Any]:
    return {
        'terrain': {
            'U': 'Urban (gray)',
            'R': 'Road (blue-cyan)',
            'S': 'Structure (light gray)',
            'G': 'Green/park (green)',
            'W': 'Water (blue)'
        },
        'overlays': {
            'hazard': 'Red overlay; opacity ~ intensity (0..1)',
            'victim': 'Orange pin icon',
            'resource': 'Green square',
            'team': 'White rescuer icon'
        }
    }
