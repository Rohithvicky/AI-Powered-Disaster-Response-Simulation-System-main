...
class SimulationDashboard {
    ...
    drawGrid() {
        ...
        // Draw terrain with sophisticated colors
        if (this.simulationState.grid) {
            for (let i = 0; i < gridSize; i++) {
                for (let j = 0; j < gridSize; j++) {
                    ...
                    let color = '#1e293b'; // Dark slate for default
                    if (terrain === 'G') color = '#064e3b'; // Deep forest green
                    else if (terrain === 'W') color = '#0c4a6e'; // Deep ocean blue
                    else if (terrain === 'R') color = '#7c2d12'; // Deep rust brown
                    else if (terrain === 'U') color = '#374151'; // Dark urban gray
                    
                    // Create subtle gradient for each cell
                    const cellGradient = ctx.createRadialGradient(
                        x + cellSize/2, y + cellSize/2, 0,
                        x + cellSize/2, y + cellSize/2, cellSize/2
                    );
                    cellGradient.addColorStop(0, color);
                    cellGradient.addColorStop(1, this._darkenColor(color, 0.3));
                    
                    ctx.fillStyle = cellGradient;
                    ctx.fillRect(x + 1, y + 1, cellSize - 2, cellSize - 2);
                    
                    // Add subtle border
                    ctx.strokeStyle = this._lightenColor(color, 0.2);
                    ctx.lineWidth = 0.5;
        ...
        // Draw hazards with sophisticated colors and effects
        if (this.simulationState.hazards) {
            this.simulationState.hazards.forEach(hazard => {
                ...
                if (intensity > 0.2) {
                    ...
                    // Create sophisticated hazard gradient
                    const hazardGradient = ctx.createRadialGradient(
                        x + cellSize/2, y + cellSize/2, 0,
                        x + cellSize/2, y + cellSize/2, cellSize/2
                    );
                    ...
                    if (intensity > 0.8) {
                        baseColor = '#dc2626'; // Deep red
                        glowColor = '#fca5a5'; // Light red glow
                    } else if (intensity > 0.5) {
                        baseColor = '#ea580c'; // Orange-red
                        glowColor = '#fed7aa'; // Light orange glow
                    } else {
                        baseColor = '#d97706'; // Amber
                        glowColor = '#fde68a'; // Light amber glow
                    }
                    
                    hazardGradient.addColorStop(0, baseColor);
                    hazardGradient.addColorStop(0.7, this._darkenColor(baseColor, 0.3));
                    hazardGradient.addColorStop(1, this._darkenColor(baseColor, 0.6));
                    ...
                    ctx.strokeStyle = this._lightenColor(baseColor, 0.4);
                    ctx.lineWidth = 1.5;
                    ctx.strokeRect(x + 1, y + 1, cellSize - 2, cellSize - 2);
                }
            });
        ...
        // Draw victims with sophisticated styling
        if (this.simulationState.victims) {
            this.simulationState.victims.forEach(victim => {
                ...
                // Create sophisticated victim gradient
                const victimGradient = ctx.createRadialGradient(
                    x + cellSize/2, y + cellSize/2, 0,
                    x + cellSize/2, y + cellSize/2, cellSize/2
                );
                victimGradient.addColorStop(0, '#dc2626');
                victimGradient.addColorStop(0.7, '#991b1b');
                victimGradient.addColorStop(1, '#7f1d1d');
                
                ctx.fillStyle = victimGradient;
                ...
                // Draw victim icon with enhanced visibility
                this.drawIcon(ctx, 'victim-icon', x, y, cellSize);
                
                // Add sophisticated border with glow
        ...
        // Draw resources with sophisticated styling
        if (this.simulationState.resources) {
            this.simulationState.resources.forEach(resource => {
                ...
                // Create sophisticated resource gradient
                const resourceGradient = ctx.createRadialGradient(
                    x + cellSize/2, y + cellSize/2, 0,
                    x + cellSize/2, y + cellSize/2, cellSize/2
                );
                
                let baseColor, glowColor, borderColor;
                if (resourceType.startsWith('used_')) {
                    baseColor = '#4b5563'; // Dark gray for used
                    glowColor = '#9ca3af'; // Light gray glow
                    borderColor = '#6b7280';
                } else {
                    baseColor = '#059669'; // Deep green for available
                    glowColor = '#6ee7b7'; // Light green glow
                    borderColor = '#10b981';
                }
                
                resourceGradient.addColorStop(0, baseColor);
                resourceGradient.addColorStop(0.7, this._darkenColor(baseColor, 0.2));
                resourceGradient.addColorStop(1, this._darkenColor(baseColor, 0.4));
        ...
        // Draw recommended path with sophisticated styling
        if (this.recommendedPath && this.recommendedPath.length > 1) {
            // Draw path with sophisticated glow effect
        ...
        // Draw rescue team with sophisticated styling
        if (this.simulationState.rescue_team) {
            const [i, j] = this.simulationState.rescue_team.position;
            const x = j * cellSize;
            const y = i * cellSize;
            
            // Create sophisticated rescue team gradient
            const rescueGradient = ctx.createRadialGradient(
                x + cellSize/2, y + cellSize/2, 0,
                x + cellSize/2, y + cellSize/2, cellSize/2
            );
            rescueGradient.addColorStop(0, '#3b82f6');
            rescueGradient.addColorStop(0.7, '#1e40af');
            rescueGradient.addColorStop(1, '#1e3a8a');
            
            ctx.fillStyle = rescueGradient;
            ctx.fillRect(x + 2, y + 2, cellSize - 4, cellSize - 4);
    ...
    drawIcon(ctx, icon, x, y, size) {
        ...
        // Set font and draw emoji with glow effect
