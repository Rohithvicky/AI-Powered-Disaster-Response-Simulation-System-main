...
function draw() {
  ...
  for (const [i,j] of state.victims) {
    ...
    if (window.__showSurvival && state.survival_probabilities){
      const rec = state.survival_probabilities.find(p=>p[0]===i && p[1]===j);
      if (rec){ const p = Math.round(rec[2]*100); ctx.fillStyle = '#ffffff'; ctx.font = `${Math.max(10, Math.floor(cellSize*0.25))}px Arial`; ctx.fillText(`${p}%`, j*cellSize+cellSize*0.3, i*cellSize+cellSize*0.65); }
    }
  }
...
const survivalToggle = document.createElement('button');
survivalToggle.textContent = 'Toggle Survival%';
survivalToggle.onclick = ()=>{ window.__showSurvival = !window.__showSurvival; draw(); };
document.querySelector('.controls').insertBefore(survivalToggle, document.getElementById('btnRecommend'));
