...
function draw() {
  ...
  // Hazards as soft gradient overlay
  ...
  for (const [i,j] of state.victims) {
    const vx = j*cellSize+cellSize*0.1, vy = i*cellSize+cellSize*0.05; try { ctx.drawImage(victimImg, vx, vy, cellSize*0.8, cellSize*0.9); } catch {}
  }
...
async function refresh() { const data = await safe(()=>api.getState(), 'getState'); if (!data) return; state = data.state; updateStats(data.stats); setStatus(`Step ${state.time_step}`); draw(); }

async function autoStepOnce(){ const data = await safe(()=>api.step(),'step'); if (!data) return false; state = data.state; updateStats(data.stats); setStatus(`Step ${state.time_step}${data.done?' (done)':''}`); draw(); return !data.done; }
...
document.getElementById('btnRecommend').onclick = async () => { const data = await safe(()=>api.recommend(),'recommend'); if (!data) return; const path = data.path || []; logEl.textContent += `Recommended path len=${path.length}\n`; ctx.strokeStyle = '#eab308'; ctx.lineWidth = 2; ctx.beginPath(); for (let k=0;k<path.length;k++){ const [i,j] = path[k]; const x = j*cellSize+cellSize/2, y = i*cellSize+cellSize/2; if (k===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); } ctx.stroke(); };
