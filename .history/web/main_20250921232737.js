...
class SimulationDashboard {
    ...
    async recommendPath() {
        try {
            ...
            // Generate a simple path recommendation
            const rescueTeam = this.simulationState.rescue_team;
            const victims = this.simulationState.victims;
            
            if (!rescueTeam || !victims || victims.length === 0) {
                this.showNotification('No victims to rescue!', 'warning');
                return;
            }
            
            // Find nearest victim
            const nearestVictim = victims.reduce((nearest, victim) => {
                const currentDist = Math.abs(victim[0] - rescueTeam.position[0]) + Math.abs(victim[1] - rescueTeam.position[1]);
                const nearestDist = Math.abs(nearest[0] - rescueTeam.position[0]) + Math.abs(nearest[1] - rescueTeam.position[1]);
                return currentDist < nearestDist ? victim : nearest;
            });
            
            // Generate path (simple straight line for now)
            const path = this.generatePath(rescueTeam.position, nearestVictim);
            this.showRecommendedPath(path);
            
            this.showNotification(`AI recommends path to victim at (${nearestVictim[0]}, ${nearestVictim[1]})`, 'success');
        } catch (error) {
