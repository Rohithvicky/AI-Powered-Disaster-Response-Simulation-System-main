...
class SoundManager {
    ...
    playRescueSound(resourceType) {
        ...
        // Always play success sound after rescue (only for non-rescue_team resources)
        if (resourceType !== 'rescue_team') {
...
class SimulationDashboard {
    constructor(apiBase = `${window.location.origin}/api`) {
        ...
        this.processedRescues = new Set(); // Track processed rescue operations
        this.iconImages = {}; // Preloaded SVG images
        this.init();
    }
    ...
    async resetSimulation() {
        try {
            ...
            const data = await response.json();
            this.simulationState = data.state;
            this.processedRescues.clear(); // Clear processed rescues on reset
            this.updateDashboard();
            this.updateLastUpdateTime();
        } catch (error) {
    ...
    async stepSimulation() {
        try {
            ...
            const data = await response.json();
            // API returns { result: {...}, state: {...} }
            if (data && data.state) {
                this.simulationState = data.state;
            } else if (data.result && data.result.state) {
    ...
    updateMetrics() {
        ...
        if (this.simulationState.rescue_team) {
            const team = this.simulationState.rescue_team;
            const energyElement = document.getElementById('teamEnergy');
            
            if (energyElement) energyElement.textContent = `${team.energy.toFixed(0)}%`;
        }
        
        // Update victim statistics
        if (this.simulationState.victims) {
            const victims = this.simulationState.victims;
            const criticalVictims = victims.filter(v => v.injury_level >= 4).length;
            const avgSurvival = victims.length > 0 ? 
                (victims.reduce((sum, v) => sum + v.survival_probability, 0) / victims.length * 100).toFixed(1) : 0;
            
            const criticalElement = document.getElementById('criticalVictims');
            const survivalElement = document.getElementById('avgSurvival');
            
            if (criticalElement) criticalElement.textContent = criticalVictims;
            if (survivalElement) survivalElement.textContent = `${avgSurvival}%`;
        }
    }
    ...
    updateAIDecisions() {
        ...
        if (resourceMovements.length > 0) {
            ...
            recentMovements.forEach(movement => {
                ...
                const riskLevel = movement.path_risk || 0;
                const pathLength = movement.path_length || 0;
                const riskText = riskLevel > 1.0 ? '⚠️ High Risk' : riskLevel > 0.5 ? '⚡ Medium Risk' : '✅ Low Risk';
                const pathText = pathLength > 0 ? ` | Path: ${pathLength} steps` : '';
                
                movementElement.className = 'decision-item';
                movementElement.innerHTML = `
                    <span class="decision-action">🚶 ${movement.resource} moved</span>
                    <span class="decision-detail">from (${movement.from[0]},${movement.from[1]}) to (${movement.to[0]},${movement.to[1]}) - ${riskText}${pathText}</span>
                `;
        ...
        if (rescueOperations.length > 0) {
            ...
            recentRescues.forEach((operation, index) => {
                // Create unique identifier for this rescue operation
                const rescueId = `${operation.step}-${operation.victim[0]}-${operation.victim[1]}`;
                ...
                // Only play sound and show notification for new rescues
                if (!this.processedRescues.has(rescueId)) {
                    this.processedRescues.add(rescueId);
                    
                    // Play sound effects for each resource used with delay
                    operation.resources_used.forEach((resource, resourceIndex) => {
                        setTimeout(() => {
                            this.soundManager.playRescueSound(resource);
                        }, resourceIndex * 200);
                    });
                    
                    // Show notification for rescue
                    const resourceNames = {
                        'ambulance': '🚑 Ambulance',
                    ...
                    const usedResourceNames = operation.resources_used.map(r => resourceNames[r] || r).join(', ');
                    this.showNotification(`🎉 VICTIM RESCUED! Used: ${usedResourceNames}`, 'success');
                }
            });
        }
    ...
    drawGrid() {
        ...
        // Draw victims with proper colors based on survival probability
        if (this.simulationState.victims) {
            this.simulationState.victims.forEach(victim => {
                const [i, j] = victim.position;
                const x = j * cellSize;
                const y = i * cellSize;
                
                // Color based on survival probability and injury level
                let baseColor, borderColor;
                if (victim.survival_probability > 0.7) {
                    baseColor = '#fbbf24'; // Yellow - good condition
                    borderColor = '#d97706';
                } else if (victim.survival_probability > 0.4) {
                    baseColor = '#f97316'; // Orange - moderate condition
                    borderColor = '#ea580c';
                } else {
                    baseColor = '#dc2626'; // Red - critical condition
                    borderColor = '#b91c1c';
                }
                ...
                // Add injury level indicator
                if (victim.injury_level >= 4) {
                    ctx.fillStyle = '#ffffff';
                    ctx.font = `${cellSize * 0.3}px Arial`;
                    ctx.textAlign = 'center';
                    ctx.fillText('!', x + cellSize/2, y + cellSize * 0.7);
                }
            });
        }
        ...
        // Draw current rescue path if available
        if (this.simulationState.metrics && this.simulationState.metrics.resource_movements) {
            const recentMovements = this.simulationState.metrics.resource_movements.slice(-1);
            if (recentMovements.length > 0) {
                const movement = recentMovements[0];
                if (movement.path_length > 1) {
                    // Draw a simple path indicator
                    const startX = movement.from[1] * cellSize + cellSize/2;
                    const startY = movement.from[0] * cellSize + cellSize/2;
                    const endX = movement.to[1] * cellSize + cellSize/2;
                    const endY = movement.to[0] * cellSize + cellSize/2;
        ...
        // Draw rescue team with proper colors based on energy and status
        if (this.simulationState.rescue_team) {
            ...
            let baseColor, borderColor;
            const energyPercent = this.simulationState.rescue_team.energy / 100;
            if (energyPercent > 0.7) {
                baseColor = '#fed7aa'; // Light orange - good energy
                borderColor = '#fb923c';
            } else if (energyPercent > 0.4) {
                baseColor = '#fbbf24'; // Yellow - moderate energy
                borderColor = '#f59e0b';
            } else {
                baseColor = '#fca5a5'; // Light red - low energy
                borderColor = '#ef4444';
            }
            ...
            const status = this.simulationState.rescue_team.status;
            if (status === "rescuing") {
                ctx.fillStyle = '#dc2626';
                ctx.font = `${cellSize * 0.3}px Arial`;
                ctx.textAlign = 'center';
                ctx.fillText('R', x + cellSize/2, y + cellSize * 0.3);
            } else if (status === "transporting") {
                ctx.fillStyle = '#2563eb';
                ctx.font = `${cellSize * 0.3}px Arial`;
                ctx.textAlign = 'center';
                ctx.fillText('T', x + cellSize/2, y + cellSize * 0.3);
            }
    ...
    preloadIcons() {
        const map = {
            'rescuer-icon': 'rescuer.svg',
            'victim-icon': 'victim.svg',
            'ambulance-icon': 'ambulance.svg',
            'fire-truck-icon': 'fire-truck.svg',
            'helicopter-icon': 'helicopter.svg',
            'boat-icon': 'boat.svg',
            'medical-icon': 'medical.svg',
            'machinery-icon': 'machinery.svg'
        };
    ...
    drawIcon(ctx, icon, x, y, size) {
        // Prefer SVG icon if loaded; otherwise fallback to emoji for compatibility
        const img = this.iconImages[icon];
    ...
    handleGridHover(event) {
        ...
        if (row >= 0 && row < gridSize && col >= 0 && col < gridSize) {
            ...
            const victim = this.simulationState.victims.find(v => Array.isArray(v) ? (v[0] === row && v[1] === col) : (v.position && v.position[0] === row && v.position[1] === col));
    ...
    initializeAudio() {
        // Initialize audio context for the sound manager
        if (this.soundManager && !this.soundManager.audioContext) {
            try {
                this.soundManager.audioContext = new (window.AudioContext || window.webkitAudioContext)();
    ...
    testSound() {
        ...
        this.showNotification('Sound test played! 🔊', 'info');
    }
}
